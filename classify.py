...
def classifyOne():
    ...
    #call model
    classifierCNN = load_model('model/classifier_cnn_lda_raw_csv_hetero_fashion_epoch100.h5py')
    ...
    plt.suptitle('predict info : {}'.format(label_dict[getIndex(predict[0])]), fontsize=16)
    plt.show()
...
def classify():
    ...
    classifierCNN = load_model('model/classifier_cnn_lda_mongodb_fashion_epoch100.h5py')
    predicted = classifierCNN.predict(Xtest)
    ...
    print('right class shape : {}'.format(Ytest1.shape))
    ...
    for i in range(10):
        ...
        plt.title("Class : {}".format(label_dict[getIndex(Ytest1[i])]))
        plt.tight_layout()
    plt.show()
"""
...
def load_data_from_csv():
    ...
    df_test_x = df_test.iloc[:, 0:784]
    df_test_y = df_test['y']
    first_image = df_test_x.loc[1, :]
    first_label = df_test_y[1]
    plt.imshow(np.reshape(first_image.values, (28, 28)), cmap='gray_r', interpolation='none')
    ...
    df_train_x = df_train.iloc[:, 0:784]
    df_train_y = df_train['y']
    first_image = df_train_x.loc[1, :]
    first_label = df_train_y[1]
    plt.imshow(np.reshape(first_image.values, (28, 28)), cmap='gray_r', interpolation='none')
    plt.title('Digit Label: {}'.format(first_label))
    plt.show()
    #standardize data
